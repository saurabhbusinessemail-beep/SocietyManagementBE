...
class DemoBookingService {
    ...
    calculateScheduledDemoDate(date, time) {
        const dateStr = new Date(date).toISOString().split('T')[0];
        const timeStr = this.convertTimeTo24Hour(time);
    ...
    async createBooking(bookingData, reqInfo = {}) {
        try {
            ...
            // Validate time slot
            if (!this.isValidTimeSlot(bookingData.preferredTime)) {
                return {
                    success: false,
            ...
            // Check availability
            const slots = await this.getAvailableSlots(bookingData.preferredDate, 3);
            ...
            const newBooking = new DemoBooking({
                ...
                scheduledDemoDate: this.calculateScheduledDemoDate(
    ...
    async getAvailableSlots(date, maxPerSlot = 3) {
        try {
    ...
    async getBookingsByDateRange(startDate, endDate, status = null) {
        try {
            const query = {
                preferredDate: {
    ...
    async rescheduleDemo(bookingId, newDate, newTime, reason, rescheduledBy = 'customer') {
        try {
            // Get booking
            const booking = await DemoBooking.findById(bookingId);
            if (!booking) {
            ...
            if (!this.isValidTimeSlot(newTime)) {
                return {
                    success: false,
            ...
            // Check availability
            const slots = await this.getAvailableSlots(newDate, 3);
            ...
            booking.scheduledDemoDate = this.calculateScheduledDemoDate(newDate, newTime);
    ...
    async completeDemo(bookingId, feedback, rating) {
        try {
            const booking = await DemoBooking.findById(bookingId);
            if (!booking) {
                return {
    ...
    async cancelDemo(bookingId, reason) {
        try {
            const booking = await DemoBooking.findById(bookingId);
            if (!booking) {
                return {
    ...
    async markAsConverted(bookingId) {
        try {
            const booking = await DemoBooking.findById(bookingId);
            if (!booking) {
                return {
    ...
    async addFollowUpNote(bookingId, note) {
        try {
            const booking = await DemoBooking.findById(bookingId);
            if (!booking) {
                return {
    ...
    async assignTo(bookingId, userId) {
        try {
            const booking = await DemoBooking.findById(bookingId);
            if (!booking) {
                return {
...
module.exports = new DemoBookingService();